...
(765 * 1 + 220 * 2 + 28 * 3 + 684 * 4 + 483 * 5). So the total winnings in this example 
are 6440.
...
function firstTask(data: string) {
  ...
  const handRanks = getHandRanks(dataArray);
  const finalHandOrder = getFinalHandArray(handRanks);

  return finalHandOrder.reduce((acc, bid, index) => {
    return acc + bid * (index + 1);
  }, 0);
}

type HandMap = { [key: string]: number };

function getHandRanks(dataArray: string[]) {
  const handRanks: { [key: number]: Hand[] } = {};
  dataArray.forEach((line) => {
    const [hand, bidString] = line.split(" ");
    const bid = parseInt(bidString);
    const handRank = getHandRank(hand);

    if (!handRanks[handRank]) {
      handRanks[handRank] = [];
    }

    handRanks[handRank].push({ hand: hand.toString(), bid });
  });
  ...
  function getHandRank(hand: string) {
    ...
    const distinctCards = Object.keys(handMap).length;
    if (distinctCards === 1) {
      return HAND_TYPES.fiveOfAKind;
    }
    if (distinctCards === 2) {
      const [firstCard, secondCard] = Object.keys(handMap);
      if (handMap[firstCard] === 4 || handMap[secondCard] === 4) {
        return HAND_TYPES.fourOfAKind;
      }
      return HAND_TYPES.fullHouse;
    }
    if (distinctCards === 3) {
      const [firstCard, secondCard, thirdCard] = Object.keys(handMap);
      if (
        handMap[firstCard] === 3 ||
        handMap[secondCard] === 3 ||
        handMap[thirdCard] === 3
      ) {
...
function getFinalHandArray(handRanks: { [key: number]: Hand[] }) {
  const finalHandOrder: number[] = [];
  Object.keys(handRanks).forEach((handRank) => {
    handRanks[parseInt(handRank)].sort(customSort);
  });

  for (let i = 6; i >= 0; i--) {
    const currentHands = handRanks[i];
    if (!currentHands) {
      continue;
    }
    finalHandOrder.push(...currentHands.map((hand) => hand.bid));
  }

  return finalHandOrder;
}

function customSort(a: Hand, b: Hand) {
  type PokerValues = { [key: string]: number };

  const POKER_VALUES: PokerValues = {
    A: 14,
    K: 13,
    Q: 12,
    J: 11,
  ...
  const [aHand, bHand] = [a.hand, b.hand];
  const [aHandArray, bHandArray] = [aHand.split(""), bHand.split("")];
  for (let i = 0; i < aHandArray.length; i++) {
    if (POKER_VALUES[aHandArray[i]] > POKER_VALUES[bHandArray[i]]) {
      return 1;
    }
    if (POKER_VALUES[aHandArray[i]] < POKER_VALUES[bHandArray[i]]) {
      return -1;
...
function secondTask(data: string) {
  const dataArray = data.split("\n");
  const handRanks = getHandRanksWithJokers(dataArray);
  const finalHandOrder = getFinalHandArrayWithJokers(handRanks);
...
function getHandRanksWithJokers(dataArray: string[]) {
  const handRanks: { [key: number]: Hand[] } = {};
  dataArray.forEach((line) => {
    const [hand, bidString] = line.split(" ");
    const bid = parseInt(bidString);
    const handRank = getHandRankWithJokers(hand);
  ...
  function getHandRankWithJokers(hand: string) {
    ...
    if (handMap["J"]) {
      const numberOfJokers = handMap["J"];
      delete handMap["J"];
      let valueWithHighestCount = 0;
      let cardWithHighestCount = "";
      Object.keys(handMap).forEach((card) => {
        if (handMap[card] > valueWithHighestCount) {
          valueWithHighestCount = handMap[card];
          cardWithHighestCount = card;
        }
      });
      handMap[cardWithHighestCount] += numberOfJokers;
    }
...
function getFinalHandArrayWithJokers(handRanks: { [key: number]: Hand[] }) {
  const finalHandOrder: number[] = [];
  Object.keys(handRanks).forEach((handRank) => {
    handRanks[parseInt(handRank)].sort(customSortWithJokers);
  });
...
function customSortWithJokers(a: Hand, b: Hand) {
  type PokerValues = { [key: string]: number };
