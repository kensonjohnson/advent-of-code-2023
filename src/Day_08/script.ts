...
function firstTask(data: string) {
  const [instructions, mapString] = data.split("\n\n");
  const instructionsArray = instructions.split("");
  const nodes = parseMap(mapString);

  let currentNode = "AAA";
  let step = 0;
  while (currentNode !== "ZZZ") {
    const direction = instructionsArray[step % instructionsArray.length] as
      | "L"
      | "R";
    currentNode = nodes[currentNode as keyof typeof nodes][direction];
    step++;
  }
  return step;
}

function parseMap(mapString: string) {
  ...
  mapString.split("\n").forEach((node) => {
    ...
    const [left, right] = connections
      .replace(/\(|\)/g, "")
      .split(", ")
      .map((connection) => connection.replace(/"/g, ""));
...
function secondTask(data: string) {
  const [instructions, mapString] = data.split("\n\n");
  const instructionsArray: ("L" | "R")[] = instructions.split("") as (
    | "L"
    | "R"
  )[];
  const nodes = parseMap(mapString);
    ...
    while (true) {
      if (
        currentNodes[currentNodeIndex][
          currentNodes[currentNodeIndex].length - 1
        ].endsWith("Z")
      ) {
        break;
      }

      currentNodes[currentNodeIndex].push(
        nodes[
          currentNodes[currentNodeIndex][
            currentNodes[currentNodeIndex].length - 1
          ]
        ][instructionsArray[i % instructionsArray.length]]
      );
      i++;
    }
  }

  const allPathLengths = currentNodes.map((node) => node.length - 1);

  return lcm(allPathLengths);
}
