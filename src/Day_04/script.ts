...
and 86) are winning numbers! That means card 1 is worth 8 points (1 for the first match, 
...
function firstTask(data: string) {
  ...
  dataArray.forEach((line) => {
    ...
    const winningNumbers = new Set(
      winningNumbersString.match(/.{1,3}/g)?.map((num) => parseInt(num))
    );
...
function secondTask(data: string) {
  ...
  dataArray.forEach((line, index) => {
    ...
    const numberOfCurrentScratchCard = dataMap[index];
    const [winningNumbersString, myNumbersString] = line.split(" | ");
    const winningNumbers = new Set(
      winningNumbersString.match(/.{1,3}/g)?.map((num) => parseInt(num))
    );
    ...
    // Calculate the bonus cards
    let bunusScratchcards = 0;
    myNumbers?.forEach((num) => {
      if (winningNumbers.has(num)) {
        bunusScratchcards++;
        dataMap[index + bunusScratchcards] =
          dataMap[index + bunusScratchcards] + numberOfCurrentScratchCard ||
          1 + numberOfCurrentScratchCard;
      }
    });
  });
