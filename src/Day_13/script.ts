...
function firstTask(data: string) {
  ...
  for (const section of sections) {
    const horizontal = findHorizonalValue(section) * 100;
    const vertical = findVerticalValue(section);
    count += horizontal + vertical;
  }
  return count;
}

function findVerticalValue(section: string[]) {
  for (let index = 0; index < section[0].length - 1; index++) {
    // Add 1 because of the 0 index
    if (isValidVertical(section, index)) return index + 1;
  }
  return 0;
}

function isValidVertical(section: string[], index: number) {
  let valid = true;
  let leftIndex = index;
  let rightIndex = index + 1;
  while (leftIndex >= 0 && rightIndex < section[0].length) {
    // Iterate down all of the rows to create the columns
    let leftColumn = section.map((row) => row[leftIndex]).join("");
    let rightColumn = section.map((row) => row[rightIndex]).join("");
...
function findHorizonalValue(section: string[]) {
  for (let index = 0; index < section.length - 1; index++) {
    // Add 1 because of the 0 index
    if (isValidHorizontal(section, index)) return index + 1;
  }
  return 0;
}

function isValidHorizontal(section: string[], index: number) {
  let valid = true;
  let topIndex = index;
  let bottomIndex = index + 1;
  while (topIndex >= 0 && bottomIndex < section.length) {
    let topRow = section[topIndex];
    let bottomRow = section[bottomIndex];
    if (topRow !== bottomRow) {
      valid = false;
      break;
    }
...
number do you get after summarizing the new reflection line in each pattern in your notes?
*/
