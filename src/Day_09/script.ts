...
puzzle input). Each line in the report contains the history of a single value. 
For example:
...
function firstTask(data: string) {
  ...
  let sum = 0;
  dataArr.forEach((stack) => {
    populateStack(stack);
    sum += predictNextValue(stack);
  });

  return sum;
}

function populateStack(stack: number[][]) {
  while (true) {
    const differences = getDifferences(stack[stack.length - 1]);
    stack.push(differences);
    if (checkIfAllZeroes(differences)) {
      break;
    }
  }
}

function getDifferences(arr: number[]) {
  const differences = [];
...
function checkIfAllZeroes(arr: number[]) {
  return arr.every((num) => num === 0);
}

function predictNextValue(stack: number[][]) {
  for (let index = stack.length - 2; index >= 0; index--) {
    const currentRow = stack[index];
    const parentRow = stack[index + 1];
    currentRow.push(
      currentRow[currentRow.length - 1] + parentRow[parentRow.length - 1]
    );
  }
...
function secondTask(data: string) {
  ...
  dataArr.forEach((stack) => {
    populateStack(stack);
    sum += predictFirstValue(stack);
  });

  return sum;
}

function predictFirstValue(stack: number[][]) {
  for (let index = stack.length - 2; index >= 0; index--) {
    const currentRow = stack[index];
    const parentRow = stack[index + 1];
    currentRow.unshift(currentRow[0] - parentRow[0]);
  }
  return stack[0][0];
}
